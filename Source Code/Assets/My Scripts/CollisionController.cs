...
public class CollisionController : MonoBehaviour {
	...
	void DescendSlope(ref Vector3 velocity){
		...
		RaycastHit2D hit = Physics2D.Raycast (rayOrigin, -(Vector2)Vector2.up, Mathf.Infinity, collisionMask);
		if (hit) {
			//print ("2");
			float slopeAngle = Vector2.Angle (hit.normal, (Vector2)Vector2.up);
			if (slopeAngle != 0 && slopeAngle <= maxDescendAngle) {
				//print("3");
				//if (Mathf.Sign ((transform.rotation*hit.normal).x) == dirX) { //idk if this is right for when rotated
				//print(hit.distance - skinwidth);
				//print(slopeAngle);
				//print ("vel: " + velocity.x.ToString());
				//print ("dist: " + (hit.distance - skinwidth).ToString());
				//print ("TanPart: " + (Mathf.Tan (slopeAngle * Mathf.Deg2Rad) * Mathf.Abs (velocity.x)).ToString());
